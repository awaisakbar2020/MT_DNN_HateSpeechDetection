...
class SANBertNetwork(nn.Module):
    def __init__(self, opt, bert_config=None, initial_from_local=False):
        ...
        self.linear = nn.Linear(256*2, 3) # add number_of_classes
        ...
        for task_id in range(len(task_def_list)):
            ...
            if task_obj is not None:
                ...
                #print("DEBUG: inside BERT init method \n executing the if condition: task_obj is not None......")
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, premise_mask=None, hyp_mask=None, task_id=0, fwd_type=0, embed=None):
        if fwd_type == 2:
            ...
            last_hidden_state_init, all_hidden_states = self.encode(None, token_type_ids, attention_mask, embed) 
        elif fwd_type == 1:
            return self.embed_encode(input_ids, token_type_ids, attention_mask)
        else:
            last_hidden_state_init, all_hidden_states = self.encode(input_ids, token_type_ids, attention_mask)
            
        print("DEBUG: Before passing to LSTM layer: ",last_hidden_state_init.shape)
        lstm_output, (h,c) = self.lstm(last_hidden_state_init) ## extract the 1st token's embeddings
        print("DEBUG: After passing through LSTM layer: ",lstm_output.shape)
        hidden = torch.cat((lstm_output[:,-1, :256],lstm_output[:,0, 256:]),dim=-1)
        print("DEBUG: Concatenated hidden states: ",hidden.shape)
        print("DEBUG: hidden.view(-1,256*2) ",(hidden.view(-1,256*2)).shape)
        last_hidden_state = self.linear(hidden.view(-1,256*2)) ### assuming that you are only using the output of the last LSTM cell to perform classification
        ...
        if task_obj is not None:
            #print("DEBUG: inside BERT forward method \n executing if task_obj is not None")
            #pooled_output = self.pooler(last_hidden_state)
            #logits = task_obj.train_forward(last_hidden_state_init, last_hidden_state, premise_mask, hyp_mask, decoder_opt, self.dropout_list[task_id], self.scoring_list[task_id])
